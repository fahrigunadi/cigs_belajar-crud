    ...
    <div class="container mt-5">
        <form action="{{ route('anak.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input value="{{ old('tanggal_lahir') }}" name="tanggal_lahir" type="date" class="form-control @error('tanggal_lahir') is-invalid @enderror">
                @error('tanggal_lahir')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="ortu_id" class="form-control @error('ortu_id') is-invalid @enderror">
                    ...
                    @foreach ($ortus as $ortu)
                        <option @selected(old('ortu') == $ortu->id) value="{{ $ortu->id }}">{{ $ortu->nama }}</option>
                    @endforeach
                </select>
                @error('ortu_id')
                    <div class="invalid-feedback">{{ $message }}</div>
